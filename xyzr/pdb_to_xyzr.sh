...
usage() {
	echo "Usage: pdb_to_xyzr.sh [-h] [-t atmtypenumbers] [pdb_file]" >&2
	exit 2
}
...
numfile=""
if [ -n "$atm_table_override" ] && [ -f "$atm_table_override" ]; then
	numfile="$atm_table_override"
elif [ -n "$PDB_TO_XYZR_TABLE" ] && [ -f "$PDB_TO_XYZR_TABLE" ]; then
	numfile="$PDB_TO_XYZR_TABLE"
elif [ -f "$(dirname "$(realpath "$0")")/atmtypenumbers" ]; then
	numfile="$(dirname "$(realpath "$0")")/atmtypenumbers"
elif [ -f "$(pwd)/atmtypenumbers" ]; then
	numfile="$(pwd)/atmtypenumbers"
else
	echo "pdb_to_xyzr.sh: unable to locate atmtypenumbers (use -t or set PDB_TO_XYZR_TABLE)" >&2
	exit 2
fi

numfile=$(realpath "$numfile")

gawk -v numfile="$numfile" 'BEGIN{
